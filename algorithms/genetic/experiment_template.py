...
import genetic_algo
import eval_network
from move_generator_genetic import GeneticMoveGenerator
...
def evaluate_elo(p, stockfish: Stockfish, game_count = 20, move_limit = 20):
    ...
    for game_number in range(game_count):
        ...
        for _ in tqdm(range(move_limit), desc = f"Playing moves for game {game_number + 1}.", ascii = False,
                      ncols = 75):
...
if __name__ == "__main__":
    ...
    agent, loss = ga.execute(tourno_fitness, eval_model, pop_size = 10, generations = 15)
    print(agent.fitness)

    # experiment
    p = GeneticMoveGenerator(agent)
    sf = Stockfish(path = r"algorithms\stockfish-11-win\Windows\stockfish_20011801_x64.exe")
    sf.set_elo_rating(STOCKFISH_ELO)
    sf.set_skill_level(0)
    res = evaluate_elo(p, sf, 10, 30)
    print(res)
    with open("../../genetic2.json", "w") as f:
        json.dump(res, f)
